...
%==========================================================================
    ...
    while numel(find(isfinite(Z)))
        ...
        if STAT ~= 'P'
            ...
            %--- regions are sorted in descending order of max(T).
            % N(i)     - cluster size
            % Z(i)/U   - max(T)
            % XYZ(:,i) - voxel [X Y Z] coordinates for max(T)
        ...
        if topoFDR
        [TabDat.dat{TabLin,3:12}] = deal(Pk,Qc,N(i),tdp,Pu,Qp,U,Ze,Pz,XYZmm(:,i));
        else
        [TabDat.dat{TabLin,3:12}] = deal(Pk,Qc,N(i),tdp,Pu,Qu,U,Ze,Pz,XYZmm(:,i));
        end
        TabLin = TabLin + 1;
